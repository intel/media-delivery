...
etc.). The script accepts only 8-bit I420 YUV input streams. Mind that YUV
stream will be measured using both ffmpeg-qsv and Media SDK Sample Encode, but a stream
encapsulated in a container (e.g. MP4, TS, etc.) will be measured only using
ffmpeg-qsv (since Sample Encode does not support compressed streams). On the other hand,
a raw compressed bitstream (e.g. standard compliant h264) will be measured using both
ffmpeg-qsv and Media SDK Sample Multi Transcode.
