...
------------
+
I this case benchmark is run only for the specified stream. If stream is YUV you need to
...
OUTPUT ARTIFACTS
----------------
Script outputs data in the current folder. Artifacts are encoded (transcoded)
...
--run-ffmpeg yes|no::
	Run ffmpeg benchmark (this is default).

--run-smt yes|no::
	Run smt benchmark (this is default).
